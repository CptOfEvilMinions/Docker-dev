...
then
    ...
    sed "s#{{ LDAP_URL }}#$(cat /run/secrets/vault-example-local-ldap-bind-url)#g" | \
    sed "s#{{ LDAP_DOMAIN_NAME }}#$(cat /run/secrets/vault-example-local-ldap-bind-url | awk -F'.' '{print $2}')#g" | \
    sed "s#{{ LDAP_DOMAIN_TLD }}#$(cat /run/secrets/vault-example-local-ldap-bind-url | awk -F'.' '{print $3}')#g" | \
    sed "s#{{ BIND_USERNAME }}#$(cat /run/secrets/vault-example-local-ldap-bind-username)#g" | \
    sed "s#{{ BIND_PASSWORD }}#$(cat /run/secrets/vault-example-local-ldap-bind-password)#g")
    ...
    echo "[+] - $(date) - Enabled Vault intermediate PKI"

    cat /run/secrets/*-intCA-cert > /tmp/int_ca_bundle.pem
    openssl rsa -in /run/secrets/*-intCA-key >> /tmp/int_ca_bundle.pem
    echo "[+] - $(date) - Created intermediate CA bundle for Vault"

    # Upload CA bundle
    # https://rcronco.github.io/lemur_vault/Vault_CA.html
    # Max TTL ten years
    vault write pki_int/config/ca pem_bundle="@/tmp/int_ca_bundle.pem"

    # Set max TTL to 87600h = 10 years
    # Set default TTL to 720h = 30 days
    vault secrets tune -default-lease-ttl=720h -max-lease-ttl=87600h pki_int
    vault write pki_int/roles/$(openssl x509 -noout -subject -in /run/secrets/*-rootCA-cert -nameopt multiline | grep commonName | awk '{print $3}' | tr . -)-role \
        allow_any_name=true \
    ...
    echo "[+] - $(date) - Uploaded intermediate CA bundle to Vault"
